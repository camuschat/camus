...
const feedsSlice = createSlice({
    ...
    reducers: {
        ...
        swapFeeds(state, { payload }: PayloadAction<{ id1: string, id2: string }>) {
            const {id1, id2} = payload;
            const idx1 = state.findIndex(feed => feed.id === id1);
            const idx2 = state.findIndex(feed => feed.id === id2);
            [state[idx1], state[idx2]] = [state[idx2], state[idx1]];
        },
        setLocalAudio(state, { payload }: PayloadAction<MediaStreamTrack | null>) {
            const track = payload;
        ...
        setLocalVideo(state, { payload }: PayloadAction<MediaStreamTrack | null>) {
            const track = payload;
            const stream = track ? new MediaStream([track]) : null;
            const localFeed = state.find(feed => feed.id === 'local');
            if (localFeed) {
